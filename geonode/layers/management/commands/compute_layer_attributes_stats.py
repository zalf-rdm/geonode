...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=None,
            help='Compute stats for a specific dataset ID'
        )
        parser.add_argument(
            '--all',
            action='store_true',
        ...
        parser.add_argument(
            '--force',
            action='store_true',
            help='Force recompute even if stats already exist'
        )
        parser.add_argument(
            '--types',
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        compute_all = options.get('all', False)
        ...
        for dataset in datasets:
            ...
            for attr in attributes:
                ...
                has_stats = (
                    attr.count is not None or
                    attr.min is not None or
                    attr.max is not None or
                    attr.average is not None
                )
