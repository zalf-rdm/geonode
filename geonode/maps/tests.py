...
from geonode.base.populate_test_data import all_public, create_models, create_single_map, remove_models
...
class TestMapForm(GeoNodeBaseTestSupport):
    ...
    def test_resource_form_is_invalid_extra_metadata_not_schema_in_settings(self):
        ...
        expected = {
            "success": False,
            "errors": ["extra_metadata: EXTRA_METADATA_SCHEMA validation schema is not available for resource map"],
        }
        self.assertDictEqual(expected, response.json())
...
class MetadataSyncUtilsTest(GeoNodeBaseTestSupport):
    ...
    def test_sync_metadata_copies_simple_fields(self):
        ...
        target = Map.objects.create(owner=self.admin, title="Target", abstract="Old abstract")
    ...
    def test_sync_metadata_makes_resources_match(self):
        ...
        target = Map.objects.create(owner=self.admin, title="Target", abstract="Different")
        sync_metadata(self.map_obj, target)
        target.refresh_from_db()
    ...
    def test_sync_metadata_with_field_names_subset(self):
        ...
        target = Map.objects.create(owner=self.admin, title="Target", abstract="Old abstract", purpose="Old purpose")
    ...
    def test_sync_metadata_field_names_empty_list_syncs_nothing(self):
        ...
        target = Map.objects.create(owner=self.admin, title="Target", abstract="Original abstract")
...
class MetadataSyncViewTest(GeoNodeBaseTestSupport):
    ...
    def setUp(self):
        super().setUp()
        self.admin = get_user_model().objects.get(username="admin")
        self.non_admin = get_user_model().objects.create(username="testuser_sync", is_active=True)
    ...
    # Access control
    # ------------------------------------------------------------------ #
    def test_sync_anonymous_redirected_to_login(self):
    ...
    # GET behaviour
    # ------------------------------------------------------------------ #
    def test_sync_get_context_contains_comparison_data(self):
    ...
    def test_sync_get_with_linked_resource(self):
    ...
    # POST behaviour
    # ------------------------------------------------------------------ #
    def test_sync_post_syncs_selected_resources(self):
    ...
    def test_sync_post_without_selection_shows_warning(self):
        """POST without selecting any resources redirects (with warning)."""
        self.client.login(username="admin", password="admin")
        response = self.client.post(self._url(), data={})
        self.assertEqual(response.status_code, 302)

    def test_sync_post_with_field_names_only_syncs_selected_fields(self):
        """POST with field_names should only sync the specified fields."""
        dataset = Dataset.objects.all().first()
        if dataset is None:
            self.skipTest("No datasets available in test data")
        original_purpose = dataset.purpose
