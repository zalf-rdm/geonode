...
def update(ctx):
    ...
    envs = {
        "agrovoc_download_url": os.environ.get(
            "GEONODE_AGROVOC_URL", "https://agrovoc.fao.org/latestAgrovoc/agrovoc_core.nt.zip"
        ),
        "inspire_download_url": os.environ.get(
            "GEONODE_INSPIRE_URL",
            "https://raw.githubusercontent.com/geonetwork/core-geonetwork/master/web/src/test/resources/thesaurus/external/thesauri/theme/httpinspireeceuropaeutheme-theme.rdf",
        ),
...
def initzalf(ctx):
    ...
    ctx.run(
        f"python manage.py loaddata geonode/base/fixtures/zalfinit.json \
    --settings={_localsettings()}",
        pty=True,
    )
